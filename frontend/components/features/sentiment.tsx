...
import { Lock, Unlock, Send, Loader2, Database, User, Terminal as TerminalIcon, ChevronDown, ChevronUp, CheckCircle2, Smile, Meh, Frown } from "lucide-react"
import { motion, AnimatePresence } from "framer-motion"
import { useSentimentFHE, LogEntry } from "@/hooks/use-sentiment-fhe"
...
export function SentimentAnalysis() {
  ...
  // Hook
  const { logs, addLog, isProcessing, prediction, noiseText, encryptInput, sendToServer, decryptResult, resetState } = useSentimentFHE()
  ...
  const onEncrypt = async () => {
    if (inputText.trim().length === 0) {
        addLog("Please enter some text first.", "warning")
        return
    }
    const success = await encryptInput(inputText)
    if (success) {
  ...
  const getSentimentIcon = () => {
    if (prediction === "Positive") return <Smile className="w-16 h-16 text-green-500" />
    if (prediction === "Negative") return <Frown className="w-16 h-16 text-red-500" />
    ...
    <div className="container-custom">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="flex flex-col items-center gap-6">
                        <div className="w-full max-w-2xl">
                            <Textarea
                                ...
                                placeholder="Enter text to analyze sentiment... (e.g., 'This movie was absolutely fantastic!')"
                                className="min-h-[200px] text-base resize-none"
                            />
                            <p className="text-xs text-muted-foreground mt-2 text-center">
                                Your text will be encrypted locally before being sent to the server
                            </p>
                        </div>
                ...
                >
                     <div className="flex flex-col items-center gap-6">
                        <div className="p-6 bg-slate-950 rounded-xl border border-slate-800 shadow-inner w-full max-w-2xl">
                            {noiseText ? (
                                <pre className="text-green-400 text-xs font-mono leading-relaxed opacity-80 whitespace-pre-wrap break-all">
                                    {noiseText}
                                </pre>
                            ) : (
                        ...
                        <p className="text-sm text-muted-foreground text-center max-w-md">
                            This encrypted data represents your text. The server can perform sentiment analysis on this data but cannot read the original text.
                        </p>
                         <div className="w-full max-w-xs">
                             <Button 
                                onClick={onSend} 
                        ...
                        <div className="flex items-center justify-center min-h-[200px]">
                            {completedSteps.includes(3) && prediction !== null ? (
                                ...
                                >
                                    ...
                                    <p className="text-sm text-muted-foreground mb-2 uppercase tracking-widest">Sentiment Detected</p>
